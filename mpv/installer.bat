...
set icon_path=%~dp0\installer\icon.ico
...
:: Installing fonts
copy Cabin-Bold.ttf %systemroot%\fonts
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Fonts" /v "Cabin (OpenType)" /t REG_SZ /d Cabin-Bold.ttf /f
...
echo You can now configure mpv's file associations from Control Panel's Default Programs.
echo.
